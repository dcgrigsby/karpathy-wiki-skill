---
name: karpathy-wiki
description: Operate a Karpathy-style LLM-maintained wiki — a personal knowledge base where source material in `raw/` is processed into a cross-linked synthesis in `wiki/`. Use when the user is in a karpathy-wiki repo and asks to ingest a source ("ingest this article", "process raw/foo.md"), query the wiki ("what does the wiki say about X"), lint it ("audit for orphans"), unwind a source ("unwind <slug>"), or otherwise operate on the raw/wiki/log/index structure. Owns the conventions — page format and types (source/entity/concept/synthesis/question/overview), wiki-links, source enrichment (arxiv abs→html, podcast transcripts), multi-page sources, the security boundary around untrusted raw/ content, the qmd index, and wiki/log.md. Run from the wiki repo root; no multi-wiki resolution. If the user is editing notes in an Obsidian vault that isn't a karpathy-wiki, use the `obsidian` skill instead.
---
